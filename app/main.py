...
async def plot(request: Request, ticker: str = Form(...), start_date: str = Form(...), end_date: str = Form(...)):
    return render_plot_page(request, ticker.upper(), start_date=start_date, end_date=end_date, date_range="custom")

@app.get("/plot/{ticker}/{date_range}", response_class=HTMLResponse)
async def plot_with_range(request: Request, ticker: str, date_range: str):
    ...
    if date_range == 'MAX':
        ...
        return HTMLResponse(content="Invalid date range", status_code=400)

    return render_plot_page(request, ticker, start_date=start_date.strftime('%Y-%m-%d') if start_date else None, end_date=end_date.strftime('%Y-%m-%d') if end_date else None, date_range=date_range)